...
export function createWalletTracingConfig(env: Env): TracingConfig {
  const endpoint = env.OTEL_EXPORTER_OTLP_ENDPOINT;

  return {
    enabled: Boolean(endpoint),
    serviceName: env.OTEL_SERVICE_NAME ?? "wallets",
    tracesUrl: `${endpoint?.replace(/\/$/, "") ?? ""}/v1/traces`,
  };
}

export function startWalletTracing(env: Env = process.env): NodeSDK | null {
  const config = createWalletTracingConfig(env);
...
export function registerWalletTracingShutdown(
  sdk: ShutdownTarget,
